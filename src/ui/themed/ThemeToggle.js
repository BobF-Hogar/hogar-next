...
function ThemeToggle({ value, labels, changeFunc, style }) {
    const theme = useSelector(themeSelectors.theme);

    const [toggled, setToggled] = useState(labels ? labels.indexOf(value) > 0 : value);

    const pinRef = useRef(null);

    useEffect(() => {
        setToggled(labels ? labels.indexOf(value) > 0 : value);
    ...
    const doChange = () => {
        if (changeFunc) {
            if (labels) {
                const index = Math.max(labels.indexOf(value), 0);
